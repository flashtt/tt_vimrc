...
" Format the status line (abandoned by using vim-airline)
" set statusline=%m\ %{HasPaste()}%F%m%r%h\ %w\ \ CWD:\ %r%{getcwd()}%h
" set statusline+=\ %=
...
" => General abbreviations
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
iab <buffer> xdate <c-r>=strftime("%d/%m/%y %H:%M:%S")<cr>
